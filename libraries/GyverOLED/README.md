# Данная библеотека не чистая GyverOLED, в ней изменены некоторые параметры для этого проекта
# увкличен интервал между строками, для лучшей читаемости ( вмещается 5 строк по 21 символу, в стоке 7 по 21)
